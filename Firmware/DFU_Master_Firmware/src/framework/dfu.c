    ...
    while (1) {
        ...
        if (Dfu_State == dfuDNLOAD_SYNC) {
            if (COMM_Receive(Dfu_GetStatusRequest_Buffer, DFU_GETSTATUSREQUEST_BUFFER_SIZE)) {
                if (target_comm_transmit(Dfu_GetStatusRequest_Buffer,
                        DFU_GETSTATUSREQUEST_BUFFER_SIZE) == 0) {
        ...
        if (Command_Received) {
            ...
            if (cmd->bRequest == DFU_GETSTATE) {
                ...
                num_bytes = 0;
                if (target_comm_receive((uint8_t*) state_reply, SIZEOF_DFU_State_Reply_Packet) == 0) {
                    num_bytes
                            = COMM_Transmit((uint8_t*) state_reply, SIZEOF_DFU_State_Reply_Packet);
                }
                if (num_bytes == 0) {
                    Dfu_State = dfuERROR;
                    Dfu_Status = errCOMM;
            ...
            // Handle the DFU_GETINFO command for all states
                ...
                getinfo_reply->maxAppSize = flash_size - USER_APP_START_ADDR
                        - SIZEOF_DFU_Flash_Type;

                // Copy 16-byte device ID and 16-byte UUID from DEVICE module
                DEVICE_Fill_DeviceID_UUID(&(getinfo_reply->bDeviceID[0]));

                num_bytes = 0;
                if (target_comm_receive((uint8_t*) getinfo_reply, SIZEOF_DFU_GetInfo_Reply_Packet)
                        == 0) {
                    num_bytes = COMM_Transmit((uint8_t*) getinfo_reply,
                            SIZEOF_DFU_GetInfo_Reply_Packet);
                }
            ...
            //
                ...
                DEVICE_Reset(); // TODO: Need execute target MCU board reset
            }
        }
        ...
        //if(Command_Received || Wait_For_GetStatusRequest) // remove this if statement...
        {
            ...
            case dfuIDLE: {
                ...
                if (Command_Received && cmd->bRequest == DFU_GETSTATUS) {
                    ...
                    num_bytes = 0;
                    if (target_comm_receive((uint8_t*) state_reply, SIZEOF_DFU_Status_Reply_Packet)
                            == 0) {
                        num_bytes = COMM_Transmit((uint8_t*) status_reply,
                                SIZEOF_DFU_Status_Reply_Packet);
                    }
                    if (num_bytes == 0) {
                        Dfu_State = dfuERROR;
                        Dfu_Status = errCOMM;
                ...
                if (Command_Received && cmd->bRequest == DFU_UPLOAD) {
                    ...
                    if (cmd->wValue == 0x0000) {
                        ...
                        upload_length = FILEMGR_Start_Upload((uint8_t*) &cmd->Data,
                                FILEMGR_Get_Block_Size());
                    }

                    num_bytes = 0;
                    if (target_comm_receive(cmd->Data, upload_length) == 0) {
            ...
            case dfuERROR: {
                ...
                if (Command_Received && cmd->bRequest == DFU_GETSTATUS) {
                    ...
                    num_bytes = 0;
                    if (target_comm_receive((uint8_t*) status_reply, SIZEOF_DFU_Status_Reply_Packet)
                            == 0) {
                        num_bytes = COMM_Transmit((uint8_t*) status_reply,
                                SIZEOF_DFU_Status_Reply_Packet);
                    }
            ...
            case dfuUPLOAD_IDLE: {
                ...
                if (Command_Received && cmd->bRequest == DFU_UPLOAD) {
                    ...
                    if (cmd->wValue == Dfu_Block) {
                        ...
                        upload_length = FILEMGR_Continue_Upload((uint8_t*) &cmd->Data,
                                FILEMGR_Get_Block_Size());
                    } else {
                        Dfu_State = dfuERROR;
